...
bl_info = {
    ...
    "author": "Anthony Datu",
    "doc_url": "https://github.com/tdatu/cloudinary_exporter/blob/main/README.md",
    "support": "COMMUNITY"
}
...
class CloudinaryProperty(bpy.types.PropertyGroup):
    cloud_name: bpy.props.StringProperty(name="Cloud Name", description="Cloud Name is the last value after the @ in the environment variable", maxlen=40)
    api_key: bpy.props.StringProperty(name="API Key", description="Get API Key from Cloudinary Dashboard",maxlen=15)
    upload_preset: bpy.props.StringProperty(name="Upload Preset", description="Upload Preset defined in Cloudinary Settings", maxlen=25)
    public_id: bpy.props.StringProperty(name="Public ID",description="Public ID is the media asset identifier name in Cloudinary", maxlen=60)
...
class CLOUDINARY_OT_operator(bpy.types.Operator):
    ...
    def upload(self, dest, cld_prop):
        ...
        api_url = "https://api.cloudinary.com/v1_1/{}/image/upload".format(cld_prop.cloud_name)
        ...
        payload = {}
        files = [('file',('file',open(zipfile,'rb'),'application/octet-stream'))]
    ...
    def execute(self, context):
        ...
        if check == 3:
            home_dir = os.path.expanduser('~')
            dest = home_dir + os.sep + "Documents" + os.sep + "Cloudinary3D" + os.sep + "{}".format(public_id)
...
classes = [CloudinaryProperty, CLOUDINARY_PT_main_panel, CLOUDINARY_OT_operator]


def register():
    for cls in classes:
        bpy.utils.register_class(cls)
        bpy.types.Scene.cld_prop = bpy.props.PointerProperty(type=CloudinaryProperty)
